...
void Game::Tick()
{
	...
	xChange = m_input.getXChange();	//retrieve the input for our game
	yChange = m_input.getYChange();	//retrieve the input for our game
...
void Game::Update(DX::StepTimer const& timer)
{
	...
	{
		Vector3 rotation = m_Camera01.getRotation();
		rotation.y = rotation.y -= xChange * deltaTime* m_Camera01.getRotationSpeed();
		rotation.x = rotation.x -= yChange * deltaTime * m_Camera01.getRotationSpeed();
		if (xChange > 3 ||xChange<3) {
			m_Camera01.setRotation(rotation);
			m_Camera02.setRotation(rotation);
		}
...
//	context->RSSetState(m_states->Wireframe());
	...
	SimpleMath::Matrix newPosition3 = SimpleMath::Matrix::CreateTranslation(-2.0f, -2.7f, 0.0f);
	m_world = SimpleMath::Matrix::Identity; //set world back to identity
...
void Game::CreateDeviceDependentResources()
{
	...
	m_BasicModel2.InitializeModel(device, "object.obj");
	m_BasicModel3.InitializeBox(device, 10.0f, 0.1f, 10.0f);	

	m_BasicModel4.InitializeModel(device, "cup_.obj");
	m_BasicModel5.InitializeBox(device, 3.0, 3.0, 1.0f);//box includes dimensions
	...
	//load Textures
	CreateDDSTextureFromFile(device, L"checked.dds", nullptr, m_texture1.ReleaseAndGetAddressOf());
	CreateDDSTextureFromFile(device, L"EvilDrone_Diff.dds", nullptr, m_texture2.ReleaseAndGetAddressOf());
	CreateDDSTextureFromFile(device, L"glass.dds", nullptr, m_texture3.ReleaseAndGetAddressOf());
...
void Game::AddFog(ID3D11Device* device, const wchar_t* szFileName) {
	m_model = Model::CreateFromCMO(device, szFileName, *m_fxFactory2);
	m_model->UpdateEffects([](IEffect* effect)
		{
			...
			if (lights)
			{
				...
				lights->SetLightSpecularColor(0, Colors::Blue);
